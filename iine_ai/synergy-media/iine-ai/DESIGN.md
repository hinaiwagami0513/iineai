...
colors:
  ...
  # ブランドオレンジ primary(#ef6108) を起点にした単一色相ランプ。
  # 1つの量の内訳を「多い順＝濃い順」で見せる。系列ごとに違う色相を割り当てない。
  chart-1: "#ef6108"          # = primary。最大の系列
  chart-2: "#ff971e"          # = primary-light
  chart-3: "#ffc07a"
  chart-4: "#ffdcb8"
  chart-5: "#fff2e2"          # = primary-subtle。最小の系列
...
- **単一色相のランプ**。`primary(#ef6108)` を起点に淡くしていく5段で、**多い系列ほど濃い**。
  「今月の作業時間」のように **1つの量を分け合う** グラフは、色相を分けるより濃淡で並べたほうが
  順位がそのまま読める。色覚特性がある人も明度差で追える。
- **グラフは「オレンジ＝押せるもの」の唯一の例外**（2026-08-19 決定）。青(info)は他所で面として
  使われておらず画面から浮くため却下した。**この例外を打ち消して他の色相に戻さない。**
  代わりに「押せない」ことを形で担保する: グラフ面に枠・影・`cursor:pointer` を付けない。
...
| 補助説明 | 噛み砕いてよい。ただし**長くしない**（1〜2行） | 「投稿を出す前にAIに見てもらう」 |
...
| `POINT` | 知っておいてほしい仕様・勘違いしやすい点 | `care` |
...
- **`info`（水色）は状態バッジ専用**。「作成中」「下書き」など**状態を言うとき**だけに使い、それ以外の面に撒かない（2026-08-19 決定）。
  「この情報をAIがどこで使うか」のような**説明・ヒントのチップは `primary-subtle` 背景 + `primary` 文字**で出す。
  押せないのにオレンジを使う数少ない例外なので、**チップ止まり**にする（ボタン形にしない・枠を付けない・カード全体をオレンジにしない）。
- **チップの色は「状態か / それ以外か」で決める。** 状態＝`success`(できた) / `warning`(気をつけて) /
  `destructive`(必須・エラー) / `info`(作成中・下書き)。それ以外（説明・ヒント・何を使ったかの記録・推し情報）は
